...
Status: install ok pending" | ipkg_status_update_sd $sd $pkg
	...
	if [ -f "$info_dir/$pkg.conffiles" ]; then
		for conffile in `cat $info_dir/$pkg.conffiles`; do
			if [ -f "$dest/$conffile" ] && ! echo " $old_conffiles " | grep -q " $conffile "`openssl md5 $dest/$conffile | sed 's/.* //'`; then
...
diff -u $dest/$conffile $IPKG_TMP/$pkg/data/$conffile"
				...
				if [ -n "$use_maintainers_conffile" ]; then
					local md5sum=`openssl md5 $IPKG_TMP/$pkg/data/$conffile | sed 's/.* //'`
				...
				md5sum=`openssl md5 $IPKG_TMP/$pkg/data/$conffile | sed 's/.* //'`
				new_conffiles="$new_conffiles $conffile $md5sum"
			fi
		done
	fi
	...
	for file in $files; do
		if [ -d "$dest/$file" ]; then
			...
			if echo " $conffiles " | grep -q " $file "; then
				if echo " $conffiles " | grep -q " $file "`openssl md5 $dest/$file | sed 's/.* //'`; then
					rm -f $dest/$file
				fi
			else
				rm -f $dest/$file
